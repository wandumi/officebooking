...
class HotDeskBookingNotification extends Notification
{
    use Queueable;
    ...
    {
        $type = ucfirst($this->booking['room_type'] ?? 'Hot Desk');
        ...
        return (new MailMessage)
            ->subject(ucfirst($this->eventType) . ' Hot Desk')
            ->line($statusText)
            ->action('View Booking', url("/booking-hotdesk/{$this->booking['id']}"));
    }
    ...
    public function toDatabase(object $notifiable): array
    {
        ...
        return [
            'title' => ucfirst($this->eventType) . ' Hot Desk',
            'message' => $statusText,
