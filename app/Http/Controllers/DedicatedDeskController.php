...
class DedicatedDeskController extends Controller
{
    ...
    public function adminIndex(Request $request)
    {
        ...
        $offices = Office::with(['location', 'pricing', 'category'])
                    ...
                    ->whereHas('category', function ($q) {
                        $q->where('name', 'Dedicated Desk');
                    })
                    ->orderByDesc('created_at')
                    ->paginate(10)
                    ->withQueryString(); 

        return Inertia::render('Dedicated/AdminOffice', [
            'offices' => $offices,
            'filters' => [
                'search' => $search,
            ]
    ...
    public function create()
    {
        ...
        $categories = Category::select('id', 'name')
                ->where('name', 'Dedicated Desk')
                ->get();

        return Inertia::render('Dedicated/CreateOffice', [
            'locations'     => $locations, 
            'pricings'      => $pricing,
            'amenities'     => $amenities,
    ...
    public function edit(Office $Office)
    {
        ...
        return Inertia::render('Dedicated/EditOffice', [
    ...
    public function update(Request $request, Office $Office)
    {
        ...
        return redirect()->route('admin.dedicateddesk')
            ->with('success', 'Dedicated Office has been updated successfully.');
    }
