...
const deleteamenity = () => {
    if (amenityToDelete.value) {
        router.delete(route('admin.amenities.destroy', amenityToDelete.value), {
            preserveScroll: true,
            onSuccess: () => {
                successMessage.value = 'An amenity has been deleted successfully.';
                window.scrollTo({ top: 0, behavior: 'smooth' });
            },
            onFinish: () => {
                ...
                <div class="p-2">
                        ...
                        <table class="min-w-full border border-gray-300 divide-y divide-gray-200">
                            <thead class="bg-gray-100">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-sm font-medium text-left text-gray-700">Amenity Name</th>
                                    <th class="px-6 py-3 text-sm font-medium text-left text-gray-700">Description</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr
                                    ...
                                    :key="amenity.id">
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ amenity.id }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ amenity.amenity_name }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ amenity.description }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ amenity.price }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        <div class="flex space-x-1">
                                            <button
                                                v-if="can['edit amenities']"
                                                @click="$inertia.visit(route('admin.amenities.edit', amenity.id))"
                                                class="px-2 py-1 text-sm text-white rounded bg-bluemain hover:bg-bluemain/60">
                                                Edit
                                            </button>
                                            <button
                                                v-if="can['delete amenities']"
                                                @click="confirmDelete(amenity.id)"
                    ...
                    <div class="flex items-center justify-between mt-4">
                        <div class="text-sm text-gray-600">
                            Showing
                            <span class="font-medium">{{ amenities.from }}</span>
                            to
                            <span class="font-medium">{{ amenities.to }}</span>
                            of
                            <span class="font-medium">{{ amenities.total }}</span> results
                        </div>

                        <div class="flex space-x-1">
                            <template
                                v-for="(link, index) in amenities.links"
                                :key="index">
                                <Link
                                    v-if="link.url"
                                    :href="link.url"
