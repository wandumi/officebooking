...
const deletelocation = () => {
    if (locationToDelete.value) {
        router.delete(route('admin.location.destroy', locationToDelete.value), {
            preserveScroll: true,
            onSuccess: () => {
                successMessage.value = 'An location has been deleted successfully.';
                window.scrollTo({ top: 0, behavior: 'smooth' });
            },
            onFinish: () => {
                        ...
                        <table class="min-w-full border border-gray-300 divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr
                                    ...
                                    :key="location.id">
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ location.id }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ location.name }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ location.address }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">{{ location.city }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        <div class="flex space-x-1">
                                            <button
                                                v-if="can['edit locations']"
                                                @click="$inertia.visit(route('admin.location.edit', location.id))"
                                                class="px-2 py-1 text-sm text-white rounded bg-bluemain hover:bg-bluemain/60">
                                                Edit
                                            </button>
                                            <button
                                                v-if="can['delete locations']"
